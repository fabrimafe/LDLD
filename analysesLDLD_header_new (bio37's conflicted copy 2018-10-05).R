...
f_toRCircos_links<-function(data,color)
{
if(is.na(data[1,1])){print("dataset is empty")}
res<-as.data.frame(cbind(paste0("chr",data$chrA),data$snpA-1,data$snpA,paste0("chr",data$chrB),data$snpB-1,data$snpB,color))
names(res)<-c("Chromosome","chromStart","chromEnd","Chromosome.1","chromStart.1","chromEnd.1","PlotColor")
res$chromStart<-as.integer(as.character(res$chromStart))
res$chromEnd<-as.integer(as.character(res$chromEnd))
res$chromStart.1<-as.integer(as.character(res$chromStart.1))
res$chromEnd.1<-as.integer(as.character(res$chromEnd.1))
return(res)
}

plot_links_circos<-function(data,main_tit)
{
#nB: beforehand, run
cyto.info<-UCSC.HG19.Human.CytoBandIdeogram
...
#data<-YRI.F
if(is.na(data[[3]][1,1])==FALSE){res.int<-f_toRCircos_links(data[[3]],"blue")}
if(is.na(data[[4]][1,1])==FALSE){res.syn<-f_toRCircos_links(data[[4]],"forestgreen")}
if(is.na(data[[2]][1,1])==FALSE){res.mis<-f_toRCircos_links(data[[2]],"brown1")}
if(is.na(data[[3]][1,1])==FALSE){RCircos.Link.Plot(res.int,track.num = 2, by.chromosome = FALSE)}
if(is.na(data[[4]][1,1])==FALSE){RCircos.Link.Plot(res.syn,track.num = 2, by.chromosome = FALSE)}
if(is.na(data[[2]][1,1])==FALSE){RCircos.Link.Plot(res.mis,track.num = 2, by.chromosome = FALSE)}
title(main=main_tit)
}

generate_nework_LDLD<-function(data)
{
node1<-as.numeric(paste0(data$chrA,".",data$posA))
node2<-as.numeric(paste0(data$chrB,".",data$posB))
id<-unique(c(node1,node2))
nodes<-data.frame(id)
links<-data.frame(node1,node2,data$combined_fdr)
dim(links)#149
