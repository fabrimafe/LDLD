...
if (TRUE) {
freqfiltering=0.05
myfolder<-"/mnt/scratch/fabrizio/LDLD/above95/intergenic.1000g/repl2/min5"
myfolderdropbox<-"~/Dropbox/LDLD/ipynb/figs/intergenic.1000g/march2017/repl2/min5"
#system(paste0("cat ",myfolder,"/*minilog | grep ncomparisons | awk '{print $14}' > ",myfolder,"/ncomparisons.txt"))
index_focal_pop<-0 #now I used 7 because I was looking at FINs but I guess in general it would be safer to have 0
computed_exact_pvalues<-TRUE
popnames<-c("TSI","IBS","PUR","GWD","CHB","JPT","CHS","FIN","ACB","YRI","KHV","STU")
...
myfolderdropbox_root<-"~/Dropbox/LDLD/ipynb/figs/intergenic.1000g/march2017/"
...
myfolderdropbox_root<-"~/Dropbox/LDLD/ipynb/figs/intergenic.1000g/march2017/repl"
freqfiltering<-mynamefreq/100
}
...
{
mylogpop<-list();mylogpop_pos<-list();mylogpop_neg<-list();mylogpop_pos20<-list();logpop_nAB<-list();logpop_nAB_pos<-list();logpop_nAB_neg<-list();logpop_nAB_pos20<-list();
for ( ipop in 0:11)    
    {
    print(ipop)
    ...
    if (sum(combined_pvals[["combined_fdr_pos20"]]<0.2)>0){
...
#logpop_nAB_neg<-sapply(logpop_nAB_neg, function(i) i[1:(length(i)-2)]); #only done in output in intergenic.1000g/ for snps discovery. add to everybody
...
save(logpop_nAB_pos20,file=paste0(myfolder,"/logpop_nAB_pos20.RData"))
...
for ( ipop in 0:(npopulations-1))
    ...
    mylogpop_pos_reschr<-logpop_reschr_nAB
    logpop_nAB_pos_reschr<-logpop_reschr_nAB
save(logpop_nAB_pos_reschr,file=paste0(myfolder,"/logpop_nAB_pos20_reschr",myperm,"_nAB.RData"))
save(mylogpop_pos_reschr,file=paste0(myfolder,"/mylogpop_pos20_reschr",myperm,".RData"))
}}
...
#infoplot barplots
for (mysign_logpop_nAB in c("","_pos","_neg"_"pos20"))
{
...
allpops<-unlist(sapply(1:12,function(x) unlist(log(mylogpopnAB[[x]]+0.1)/log(max(mylogpopnAB[[x]]+0.1)))[1:(length(mylogpopnAB[[x]])-2)] ))
