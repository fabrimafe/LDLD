...
gzip /mnt/scratch/fabrizio/LDLD/postproc_split.sh ${myfolder}/anal/sorted.res 
...
for ( ipop in 0:11)    
    ...
    mylogpop[[ipop+1]]<-intersect_links(mydata_temp,dataFIN[empfdr<0.05,])
...
mylogpop_reschr<-list();logpop_reschr<-list() #number of links kept fixed
for ( ipop in 0:11)
    {
    mydata_temp<-fread(paste0(myfolder,"/reschr1/logs/all.pop",ipop,".log"))
...
#save(logpop_reschr_nAB,file=paste0(myfolder,"/logpop_nAB_nlinks.RData"))
...
infoplot(ipop,mymaxk=6,mypthr=0.05,mynABdefault=FALSE,mynAB=logpop_nAB,showreal_confidence_interval=TRUE,logpop_reschr_nAB_l=logpop_reschr_nAB_l,reshufflings=0,mylog=FALSE,showmutperssample=FALSE,mypopname=popnames[ipop],finite_mixture_criterion=2) #version with real reshuffling of chromosomes
dev.off()
}
