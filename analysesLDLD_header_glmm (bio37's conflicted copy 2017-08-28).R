...
if (glmres<mypvalue_threshold ) { if (length(unique(data_temp$population))>1) {return(c(fx(data_temp),fx(data_temp0),glmres))} else {return(c(fx1(data_temp),fx(data_temp0),glmres))} } else return(c(1,1,1))}
}
#global glmm and restricted populations glm
myfx<-function(data_temp0,myfreq=0.05,mypvalue_threshold=0.05){
data_temp<-data_temp0[unlist(unname(aggregate(data_temp0$nA,by=list(data_temp0$population),function(x) if (sum(x)/length(x)>=myfreq & sum(x)/length(x)<=(1-myfreq)){return(rep(TRUE,length(x)))}else return(rep(FALSE,length(x))))[,2])),]
if ( nrow(data_temp)>0 ){
glmres<-preliminary_check_glm_nAvsnAB(data_temp$nA,data_temp$nAB,data_temp$population)
if (glmres<mypvalue_threshold ) { if (length(unique(data_temp$population))>1) {return(c(fx(data_temp0),glmres))} else {return(c(fx(data_temp0),glmres))} } else return(c(1,1))}
}
...
if (glmres<mypvalue_threshold ) { if (length(unique(data_temp$population))>1) {return(c(fx_mut(data_temp0),glmres))} else {return(c(fx_mut(data_temp0),glmres))} }  else return(c(1,1,1))}
}
