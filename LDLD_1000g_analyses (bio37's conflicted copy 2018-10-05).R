...
source("~/Dropbox/LDLD/analysesLDLD_header_new.R")
...
if (npopulations==12){
pdf(paste0(myfolderdropbox,"/barplot_sign_links.pdf"))
barplot(links_sign_by_pop_plot,names.arg=popnames,cex.names=1,col="cadetblue",ylab="n.links")
dev.off()} else {
pdf(paste0(myfolderdropbox,"/barplot_sign_links.pdf"))
barplot(links_sign_by_pop[c(12,6,5,7,11,1,2,8,3,9,10,4)],names.arg=popnames[c(12,6,5,7,11,1,2,8,3,9,10,4)],cex.names=1,col="cadetblue",ylab="n.links")
dev.off()
}
...
samples1000g<-system("zcat /mnt/sequencedb/1000Genomes/ftp/phase3/20140910/ALL.chr22.phase3_shapeit2_mvncall_integrated_v5.20130502.genotypes.vcf.gz | head -300 | grep '#CHROM' | head -1 | awk '{for (i=10; i<=NF; i++) print $i}' 2>/dev/null",intern=TRUE)
...
#mutational load as a function of nAB.
{


#convex groupings - no groups that are interrupted
{
mydataset_mutationaload_lognAB<-c()
mydataset_mutationaload_pop<-c()
mydataset_mutationaload_groups<-c()
for (i in 1:12){
print(paste0("--------------------------------------------------"))
print(paste0("population is ",i))
print(paste0("--------------------------------------------------"))
mydata_pop<-log(logpop_nAB[[i]])
mydata_pop<-mydata_pop[1:(length(mydata_pop))]
mydataset_mutationaload_lognAB<-c(mydataset_mutationaload_lognAB,unname(mydata_pop))
mydataset_mutationaload_pop<-c(mydataset_mutationaload_pop,rep(i,length(mydata_pop)))
mymodel<-FALSE; class(mymodel) <- "try-error";
mykk<-8
while (class(mymodel) == "try-error" )
{
...
mykk<-mykk - 1; } else {mymodel=list(k_from_AICc=1,model_fromAICc=list(mu=mean(mydata_pop),sigma=sd(mydata_pop),lambda=1))}
if (class(mymodel) != "try-error"){if (length(mymodel$model_fromAICc)==1){class(mymodel) = "try-error"}}
if (class(mymodel) != "try-error"){ #this part is to avoid groupings that are not convex
    jump_in_groups<-FALSE
    if (length(mymodel$model_fromAICc$posterior)!=0){
    myorder_groups<-apply(mymodel$model_fromAICc$posterior, MARGIN=1, FUN=function(x) which(x==max(x)))+(i-1)*10
    myorderedgroups<-myorder_groups[order(mydata_pop)]
    myorderedgroups_memory<-myorderedgroups[1]
    for ( zzz in 2:length(myorderedgroups) )
        { 
        if ( ! myorderedgroups[zzz] %in% myorderedgroups_memory ){ myorderedgroups_memory<-c(myorderedgroups_memory,myorderedgroups[zzz]) } else if ( myorderedgroups[zzz-1] != myorderedgroups[zzz]) { jump_in_groups<-TRUE }
        if (jump_in_groups){class(mymodel) = "try-error"}
        }
}}

};
if (length(mymodel$model_fromAICc$posterior)==0){ mydataset_mutationaload_groups_i<-rep((i-1)*10,length(mydata_pop)) } else {
    mydataset_mutationaload_groups_i<-apply(mymodel$model_fromAICc$posterior, MARGIN=1, FUN=function(x) which(x==max(x)))+(i-1)*10
}
mydataset_mutationaload_groups<-c(mydataset_mutationaload_groups,mydataset_mutationaload_groups_i)
}

mymodelgrouping<-data.frame(lognAB=mydataset_mutationaload_lognAB,pop=mydataset_mutationaload_pop,groups=mydataset_mutationaload_groups)

save(mymodelgrouping,file=paste0(myfolderdropbox,"/mymodelgrouping_convexgrouping.RData"))
load(paste0(myfolderdropbox,"/mymodelgrouping_convexgrouping.RData"))

mydatamut<-read.table("~/Dropbox/LDLD/1000genomes/mutload/tot.tab")
mydatamut<-mydatamut[,10:ncol(mydatamut)]
mydatamut<-unname(apply(mydatamut,MARGIN=2,sum))/1000
mymodelgrouping<-cbind(mymodelgrouping,mydatamut)
cor.test(mydatamut_i$lognAB,mydatamut_i$mydatamut)
par(mfrow=c(4,3))
pdf(paste0(myfolderdropbox,"/mutationalload_bygroup.pdf"))
op <- par(mfrow = c(4,3),
          oma = c(5,4,0,0) + 0.5,
          mar = c(0,0,1,1) + 0.5)
for (i in 1:12){
mydatamut_i<-mymodelgrouping[mymodelgrouping$pop==i,]
plot(mydatamut_i$lognAB,mydatamut_i$mydatamut,col=as.numeric(as.factor(as.character(mydatamut_i$groups))),pch=19)#,xlim=c(5,11))#,ylim=c(3550,5000))
myyppos=min(mydatamut_i$mydatamut)+(max(mydatamut_i$mydatamut)-min(mydatamut_i$mydatamut))*9/10
text(x=5.4,y=myyppos,labels=popnames[i])
title(xlab = "Some Categories",
      ylab = "Some Values",
      outer = TRUE, line = 3)
mymod_i<-lm(mydatamut_i$mydatamut~mydatamut_i$lognAB)
abline(coefficients(mymod_i))
}
dev.off()
...
#plot for exome only
{
load(file=paste0(myfolder,"/l_mutsamples.RData"))
mymodelgrouping$mydatamut_exome<-unlist(l_mutsamples)/1000
...
#here mydatamut_exome was mydatamut
pdf(paste0(myfolderdropbox,"/mutationalload_bygroup_onexome.pdf"))
op <- par(mfrow = c(4,3),
          oma = c(5,4,0,0) + 0.5,
          mar = c(0,0,1,1) + 0.5)
for (i in 1:12){
...
#test effects of nAB on mutational load
{
library(lme4)
contr=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=1000000000),calc.derivs=F)
summary(model_full)
coefficients(model_full)
contr = lmerControl(optimizer ="Nelder_Mead")
library(nlme) 
control=lmeControl(opt="optim")
#save(mymodelgrouping,file=paste0(myfolderdropbox,"/mymodelgrouping_alsoexome.RData"))
load(paste0(myfolderdropbox,"/mymodelgrouping_alsoexome.RData"))
mymodelgrouping$pop<-as.factor(mymodelgrouping$pop)
mymodelgrouping$groups<-as.factor(mymodelgrouping$groups)

res_Ec=lmer(mydatamut_exome ~  Main.Project.E.Centers+(1+Main.Project.E.Centers|Population),data=mymodelgrouping,REML=FALSE,control=contr)
...
model_full=lmer(mydatamut_exome ~  lognAB+(1+lognAB|groups)+(1+lognAB|pop:groups),data=mymodelgrouping,control=contr,REML=F)

model_full=lmer(mydatamut_exome ~  lognAB+(1+lognAB||pop/groups),data=mymodelgrouping,control=contr,REML=F)

model_full=lmer(mydatamut_exome ~  lognAB+pop+pop*lognAB+(1+lognAB|groups),data=mymodelgrouping,control=contr,REML=F)

model_full=lmer(mydatamut_exome ~  lognAB+(1+lognAB|pop/groups),data=mymodelgrouping,control=contr,REML=F)

model_nonAB=lmer(mydatamut_exome ~  (1+lognAB|pop),data=mymodelgrouping,control=contr,REML=F)
anova(model_nonAB,model_full)
Data: mymodelgrouping
Models:
model_nonAB: mydatamut_exome ~ (1 + lognAB | pop)
model_full: mydatamut_exome ~ lognAB + (1 + lognAB | pop/groups)
            Df    AIC    BIC  logLik deviance Chisq Chi Df  Pr(>Chisq)    
model_nonAB  5 572.05 597.57 -281.03   562.05                             
model_full   9 551.34 597.28 -266.67   533.34 28.71      4 0.000008954 ***
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

library("MuMIn")
r.squaredGLMM(model_nonAB)
...
r2.corr.mer(model_nonAB)
r2.corr.mer(model_full)
> r.squaredGLMM(model_nonAB)
     R2m      R2c
[1,]   0 0.988854
> library("MuMIn")
...
model_full=lmer(mydatamut ~  lognAB+(1+lognAB|pop/groups),data=mymodelgrouping,control=contr,REML=F)
model_full=lmer(mydatamut ~  lognAB+(1+lognAB|pop),data=mymodelgrouping,control=contr,REML=F)

model_nonAB=lmer(mydatamut ~  (1+lognAB|pop),data=mymodelgrouping,control=contr,REML=F)
anova(model_nonAB,model_full)
Data: mymodelgrouping
Models:
model_nonAB: mydatamut ~ (1 + lognAB | pop/groups)
model_full: mydatamut ~ lognAB + (1 + lognAB | pop/groups)
            Df   AIC   BIC  logLik deviance  Chisq Chi Df Pr(>Chisq)
model_nonAB  8 13005 13046 -6494.6    12989                         
model_full   9 13007 13053 -6494.5    12989 0.2195      1     0.6394
> r.squaredGLMM(model_nonAB)
     R2m       R2c
[1,]   0 0.9911101
> r.squaredGLMM(model_full)
                R2m       R2c
[1,] 0.000002030841 0.9911165

}
#older tests not requiring grouping
{
if (class(mymodel) != "try-error"){
    jump_in_groups<-FALSE
    if (length(mymodel$model_fromAICc$posterior)!=0){
    myorder_groups<-apply(mymodel$model_fromAICc$posterior, MARGIN=1, FUN=function(x) which(x==max(x)))+(i-1)*10
    myorderedgroups<-myorder_groups[order(mydata_pop)]
...
if (length(mymodel$model_fromAICc)==0){
if (length(mymodel$model_fromAICc$posterior)==0){ mydataset_mutationaload_groups_i<-rep((i-1)*10,length(mydata_pop)) } else {
    mydataset_mutationaload_groups_i<-apply(mymodel$model_fromAICc$posterior, MARGIN=1, FUN=function(x) which(x==max(x)))+(i-1)*10
    }
}
...
save(mymodelgrouping,file=paste0(myfolderdropbox,"/mymodelgrouping.RData"))
...
mymodelgrouping$groups[mymodelgrouping$pop==10]<-mydataset_mutationaload_groups_i

mymodelgrouping<-data.frame(lognAB=mydataset_mutationaload_lognAB,pop=mydataset_mutationaload_pop,groups=mydataset_mutationaload_groups)
#save(mymodelgrouping,file=paste0(myfolderdropbox,"/mymodelgrouping.RData"))

#shell code
#cat ~/Dropbox/LDLD/ipynb/*sh | grep '~/Dropbox/LDLD/1000genomes/mutload/'
#source ~/Dropbox/LDLD/scripts/nAvsnAB_scan_child_1000g_2awk.sh
#for i in `seq 1 22`;do cat /home/fabrizio_mafessoni/Dropbox/LDLD/1000genomes/mutload/chr${i}.tab  | grep mutational | reorder_samples_vcf >> /home/fabrizio_mafessoni/Dropbox/LDLD/1000genomes/mutload/tot.tab;done

load(paste0(myfolderdropbox,"/mymodelgrouping.RData"))
mydatamut<-read.table("~/Dropbox/LDLD/1000genomes/mutload/tot.tab")
mydatamut<-mydatamut[,10:ncol(mydatamut)]
mydatamut<-unname(apply(mydatamut,MARGIN=2,sum))/1000
mymodelgrouping<-cbind(mymodelgrouping,mydatamut)
...
paste <( bedtools intersect -b ~/Dropbox/LDLD/ms/GBE/TableS4.bed -a <( cat /mnt/scratch/fabrizio/LDLD/above95/coding.exons.1000g/repl2/min5/chr*tab |  awk -v OFS='\t' '{print $1,$2-1,$2,$4,$5,$7}' | sort -Vu -k1,1 -k2,2 ) ) /mnt/scratch/fabrizio/LDLD/above95/coding.exons.1000g/repl2/min5/basessign | awk '{if (length($4)==1 && length($5)==1){print}}' > /mnt/scratch/fabrizio/LDLD/above95/coding.exons.1000g/repl2/min5/basessign.bed
...
{
for MYCHR in `seq 1 22`;do
bedtools intersect -a /mnt/scratch/fabrizio/LDLD/above95/coding.exons.1000g/repl1/min5/removed1.bed -b /mnt/scratch/fabrizio/LDLD/above95/coding.exons.1000g/repl1/min5/chr${MYCHR}.vcf.gz -sorted -wo | awk -v OFS='\t' '{print $1,$2,$3,$7,$8,$9}' >> /mnt/scratch/fabrizio/LDLD/above95/ #/mnt/scratch/fabrizio/LDLD/above95/coding.exons.1000g/repl1/min5/removed1.bed.logo
